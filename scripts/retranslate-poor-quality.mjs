...
function isTechnicalTerm(text) {
  const t = text.trim();
  if (!t) return true;
  if (/^[0-9a-f]{7,40}$/.test(t)) return true;                   // Git hash (bare)
  if (/^[0-9a-f]{7,10}\s/.test(t)) return true;                  // Git hash + message (commit log)
  if (/^[A-Z][A-Z0-9_]{2,}$/.test(t)) return true;               // ENV VAR (CODEX_CI)
  if (/^(Ctrl|Alt|Shift|Cmd|Meta)[-+]\w+$/i.test(t)) return true; // Keyboard shortcut
  if (/^[a-z][a-zA-Z0-9_.-]*$/.test(t) && t.length <= 25) return true; // Single-word identifier (lowercase-start only)
  if (/^`.+`$/.test(t)) return true;                              // Entire text in backticks (code)
  if (/^[@#][\w/.-]+/.test(t)) return true;                       // @mention, #tag
  if (/^(npm|pnpm|npx|yarn|node|git|bun)\s/.test(t)) return true; // CLI commands
  if (/^SHA\d+:\s*[0-9a-f]+/i.test(t)) return true;              // SHA256/SHA1 hash strings
  if (/^[\w./-]+\s*(->|→)\s*[\w./-]+$/.test(t)) return true;     // path/cmd rename (a -> b)
  // File extension lists: "TypeScript/JavaScript: `.ts`, `.tsx`" or "Go, Rust, Java, C/C++"
